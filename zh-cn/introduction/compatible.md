...
`amap-vue2` 能够抛开高德原生 SDK 覆盖大多数场景，但对于部分定制化程度较高的场景而言，可能还是需要引入高德原生 SDK 来支持。这章将介绍如何在 amap-vue2 中使用高德 SDK。


## 实例方式

对于大多数 `amap-vue2` 组件，都有 `init` 这个 `event`，参数为高德的实例，通过这样暴露高德实例的方式，开发者能够非常自由地将原生 SDK 和 amap-vue2 结合起来使用。
...
* 确保 `amap-vue2` 的导入名不是 `AMap`，推荐 `import VueAMap from 'amap-vue2'` 避免和高德全局的 `AMap` 冲突。
